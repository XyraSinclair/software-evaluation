...
use analysis_output::{
    ...
    print_duplicates, print_shape, print_tests,
};
...
enum Command {
    ...
    /// Score weighted co-change directory modularity beside the static layout.
    CochangeLayout {
        /// Clean Git repository to profile at HEAD.
        #[arg(default_value = ".")]
        repository: PathBuf,
    ...
    /// Profile per-function interface depth, complexity gap, nesting, and branch symmetry.
    Shape {
        #[arg(default_value = ".")]
        path: PathBuf,
        /// Maximum function rows shown in text output.
        #[arg(long, default_value_t = 30)]
        top: usize,
        #[arg(long, value_enum, default_value_t = OutputFormat::Text)]
        format: OutputFormat,
...
fn run(cli: Cli) -> Result<ExitCode, String> {
    match cli.command {
        ...
        Command::CochangeLayout {
            ...
            let report =
                analyze_cochange_layout(&repository, CochangeLayoutConfig { history_commits })
                    .map_err(|error| error.to_string())?;
            match format {
                OutputFormat::Json => print_json(&report)?,
        ...
        Command::Shape { path, top, format } => {
            let report = analyze_shape(&path).map_err(|error| error.to_string())?;
            match format {
                OutputFormat::Json => print_json(&report)?,
                OutputFormat::Text => print_shape(&report, top),
            }
            Ok(ExitCode::SUCCESS)
        }
        Command::Bench {
