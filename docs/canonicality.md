...
| Repository comparisons bind both sides to explicit commits and reject tracked worktree drift | covered | `repo-profile`/`repo-compare` evidence plus `tests/repo.rs`. |
...
| Exact, proxy, and judgment evidence cannot be confused in stored records | named-gap | Criterion evidence type epistemic class, but one-off source JSON does not yet emit a uniform record envelope. |
...
| Deterministic outputs are stable under repeated execution at one artifact commit | named-gap | Determinism is designed and unit-tested locally; a committed repeat-run equality evidence is still absent. |
...
| Branch comparison has a literal, reproducible workflow from refs to clean snapshots to evidence | named-gap | `repo-compare` requires clean worktrees; README does not yet show the branch/worktree workflow. |
...
| Property | State | Evidence or gap |
|---|---|---|
| All nine taxonomy axes are labeled evaluated, waived with reason, or uncovered in each final report | named-gap | Mechanical branch evidence cover proxies for parsimony, interface shape, correctness machinery, and evolvability; judged and empirical axes remain to be run. |
| Mechanical evidence includes denominators, distributions/tails where relevant, and raw evidence identities | named-gap | Repository programs emit Git argv/digests; one-off source instruments emit denominators and tails but not a uniform commit/tree-linked evidence. |
...
| Accepted and rejected review findings retain reasons and evidence | covered | `evaluations/exopriors-main-vs-nucleus-20260710/findings.md` records fixed, accepted-open, and ruled-out findings with closure conditions. |
