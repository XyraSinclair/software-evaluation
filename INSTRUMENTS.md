...
| Program | Observation denominator | What it can establish | What it cannot establish |
|---|---|---|---|
| `seval look PATH [--base REV]` | non-test function spaces (tangle, asymmetry), lexically public names (dead-public), tracked files when PATH is a git worktree (bulk), supported source files (squash), and files `cargo fmt` visits (format-drift) | per lens, the nearest-rank p90 tail of its own candidates with an inline reading and false-positive class; files cited by two or more lens tails, ordered by lens count; with `--base`, head tail rows absent or worse at `REV`, joined on path + name + in-file ordinal; every unavailable lens carries its reason | that a cited place is wrong or worth changing, any composite ranking, naming quality, comment truth, or anything the underlying instruments disclaim |
...
| `seval deps PATH` | import/dependency declarations in recognized source plus direct dependency rows in Cargo, npm, Python, requirements, and Go manifests | declaration evidence, coeffect grading of declared import width, conservative internal resolution, a two-tier manifest-usage join (declared dependencies vs import declarations and whole-word lexical mentions, with unused-candidates claimed only when both tiers are empty), all-edge and resolved-internal fan-in/out, SCCs, cycles, components, condensation depth, bounded exact non-self transitive reachability, exact improved trophic incoherence, directory modularity, and a detected-community witness | runtime symbol use/loading, feature/alias/build-condition resolution, transitive/lockfile dependencies, causal change impact, an optimal community partition, or architectural quality |
| `seval twins PATH` | analyzed files and unique resolved internal dependency edges from the same walker as `seval deps` | exact open/closed structural-twin classes (identical in/out neighborhoods; closed classes are mutually linked siblings), a distinct-neighbor class floor with a suppression ledger that closes the count, near-twin pairs at a declared exact Jaccard threshold with difference witnesses, and twin-membership fraction over analyzed files | semantic duplication, whether consolidation is desirable (plugin registries and fixture families legitimately twin), or twins of the true graph beyond the resolver's blind spots |
| `seval liveness PATH` | named definitions (functions, methods, types, consts) in recognized source files, mentioned against every identifier-shaped leaf plus Rust `{name}` format interpolations | per defined name: exact mention counts split by test-classified files, dead-private and dead-public candidates, single-use names with their one mention site, test-only names, and declared exclusions (entry points, dunders, test definitions and attributes, Rust trait impls) | resolved references (any same-named binding counts as liveness — deadness is never invented, liveness is over-stated), names referenced only through plain strings, external consumers of public definitions, or attribution among same-named definitions |
| `seval guards PATH` | branch/loop condition positions (if/while/for/do/ternary) in recognized source files, normalized to the clone detector's placeholder convention | repeated predicate patterns at declared occurrence/token floors with every occurrence, distinct raw spellings, the verbatim-copy maximum inside each group, and the repeated-occurrence fraction over considered conditions | that repetition is wrong (idiomatic checks repeat legitimately), guards in match arms/assertions/boolean assignments, or semantic equivalence beyond token normalization |
